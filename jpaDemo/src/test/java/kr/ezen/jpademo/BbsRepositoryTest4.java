...
//// 2. 쿼리 작성
//        JPAQuery<Bbs> query = jqf.selectFrom(bbs).where(bbs.title.eq("title7"));
//// 3. 쿼리 실행해서 조회결과 얻기
...
//        JPAQueryFactory jqf = new JPAQueryFactory(em);
//        JPAQuery<Tuple> query = jqf.select(bbs.writer, bbs.viewCnt.sum()).from(bbs)
...
//
//        JPAQueryFactory jqf = new JPAQueryFactory(em);
//        JPAQuery<Bbs> query = jqf.selectFrom(bbs).where(builder).orderBy(bbs.upDate.desc());
//        List<Bbs> list = query.fetch();
//        list.forEach(System.out::println);
//    }
}